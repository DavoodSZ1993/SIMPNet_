...
class MessagePassingNet(MessagePassing):
    def __init__(self, node_feature, aggr: str="mean", batch_norm: bool = False, **kwargs):
        ...
        self.lin_0 = nn.Sequential(
            nn.Linear(node_feature, node_feature), nn.ReLU(), nn.Dropout(),
            nn.Linear(node_feature, node_feature), nn.Dropout())                # This can be used before message passing (in message method!)
        self.lin_1 = nn.Linear(2*node_feature, node_feature)      # This can be done after the messgae passing (In the update method!)
    ...
    def forward(self, x, edge_index):
        ...
        return self.lin_1(out) # This also can be done in the update method, or can be done here!
...
class NodeEmbedding(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(in_features, 256), nn.PReLU(), nn.Dropout(),
...
class MotionGenerator(nn.Module):
    def __init__(self, node_feature_size, obs_feature_size, output_size, batch_size, num_nodes):
        ...
        self.node_embed = NodeEmbedding(node_feature_size, 64)
        self.obs_embed = ObstacleEmbedding(obs_feature_size, 64)
        ...
        self.cross_attentions = nn.ModuleList([TransformerBlock(64 * self.num_nodes, 64) for _ in range(1)])
        ...
        self.message_passing = MessagePassingNet(node_feature=64)
        ...
        self.final_linear = nn.Linear(64, output_size)
    ...
    def forward(self, graph, obstacles):
        ...
        graph.x = self.message_passing(x, edge_index)
        graph.x = self.message_passing(x, edge_index)
        #print(f'The shape of the graph: {graph.shape}')

        # Step IV - Final embedding
        joint_angles = self.final_linear(graph.x)
