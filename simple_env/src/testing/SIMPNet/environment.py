...
class LoadEnvironment:
    ...
    def add_desks(self):
        ...
        self.scene.add_box(desk3_name, desk3_pose, size=(0.5, 2.4, 1)) # Actual size of the desk in the lab!
        ...
        desktop_pose.pose.position.x = 0.15
        desktop_pose.pose.position.y = -0.45
        desktop_pose.pose.position.z = 0.55
        desktop_name = "Desktop"
        self.scene.add_box(desktop_name, desktop_pose, size=self.obs_embeddings['Desktop']['size'])

        #Add desktop II
        desktopII_pose = geometry_msgs.msg.PoseStamped()
        desktopII_pose.header.frame_id = self.robot.get_planning_frame()
        desktopII_pose.pose.orientation.w = 1.0
        desktopII_pose.pose.position.x = 0.15
        desktopII_pose.pose.position.y = 0.55
        desktopII_pose.pose.position.z = 0.55
        desktopII_name = "Desktop II"
        self.scene.add_box(desktopII_name, desktopII_pose, size=self.obs_embeddings['Desktop']['size'])

        # Add Disassembly-container II
        disassemblyII_pose = geometry_msgs.msg.PoseStamped()
        disassemblyII_pose.header.frame_id = self.robot.get_planning_frame()
        disassemblyII_pose.pose.orientation.w = 1.0
        disassemblyII_pose.pose.position.x = 0.0
        disassemblyII_pose.pose.position.y = 0
        disassemblyII_pose.pose.position.z = 0.8
        disassemblyII_name = "Disassembly_container_II"
        self.scene.add_box(disassemblyII_name, disassemblyII_pose, size=self.obs_embeddings['Disassembly_container']['size'])'''
