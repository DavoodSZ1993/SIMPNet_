...
class OnlinePlanning:
    def __init__(self, env_path:str, known_environment: bool, dof, step_size):
        ...
        if known_environment:
            self.env_path = os.path.join(self.env_path, 'known_environment')
        else:
            self.env_path = os.path.join(self.env_path, 'unknown_environment')
    ...
    def steer(self, start, end, step_size, dof, col_times):
        '''
        ...
        if dist_total > 0:
            ...
            path.append(list(state_curr))

            for i in range(num_segments):
                collision, col_times = IsInCollision(state_curr, col_times)
                if collision:
                    print('Collision detected!')
                    return False, col_times, None

                for j in range(dof):
                    state_curr[j] = state_curr[j] + dists[j]
                path.append(list(state_curr))
            
            collision, col_times = IsInCollision(end, col_times)
            if collision:
                print('Collision detected!')
                return False, col_times, None

        print('No Collision is detected!')
        return True, col_times, path

    def wrap_angle(self, theta):
        return [(t + np.pi) % (2 * np.pi) - np.pi for t in theta]

    def check_feasibility_entire_path(self, path, col_times):
        '''
        Checks the feasibility of entire path including the path edges
        '''

        for i in range(0, len(path)-1):
            ind = self.steer(path[i], path[i+1], self.step_size, self.dof, col_times)
            if not ind:
                return False
        return True
    ...
    def lazy_vertex_contraction(self, path, step_size, col_times):
        '''
        ...
        for i in range(0, len(path) - 1):
            for j in range(len(path)-1, i+1, -1):
                ind = False
                ind = self.steer(path[i], path[j], self.step_size, self.dof, col_times)
                if ind == True:
                    pc = []
    ...
    def replan_path(self, model, known_environment, config_save_path, gaussian_normalization, p, g, step_size):
        '''
