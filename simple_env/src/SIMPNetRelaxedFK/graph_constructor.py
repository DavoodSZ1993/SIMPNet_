...
class CustomDataset(Dataset):
    def __init__(self, path:str, num_ws:int, ws_names:list, ws_num_paths:dict, gaussian_normalization,
    ...
    def data_pre_proccess(self):
        ...
        for i in range(self.num_ws):
            ...
            for j in range(ws_num_paths):
                try:
                    with open(os.path.join(ws_path, f'RRTstar_raw_{j}.pkl'), 'rb') as file:
            ...
            self.normalize_dataset(ws_paths, ws_obs_embeddings, ws_name)

    def normalize_dataset(self, ws_paths, ws_obs, ws_name):
        '''
        ...
        for j in range(len(ws_paths)):
            ...
            for i in range(len(path) - 1):
                node_embed_ang_sin = torch.sin(self.wrap_angle(torch.tensor(path[i], dtype=torch.float32)).unsqueeze(1))
                node_embed_ang_cos = torch.cos(self.wrap_angle(torch.tensor(path[i], dtype=torch.float32)).unsqueeze(1))
                goal_embed_ang_sin = torch.sin(self.wrap_angle(torch.tensor(path[-1], dtype=torch.float32)).unsqueeze(1))
                goal_embed_ang_cos = torch.cos(self.wrap_angle(torch.tensor(path[-1], dtype=torch.float32)).unsqueeze(1))
                ...
                feature_matrix = torch.cat((node_embed_ang_sin, node_embed_ang_cos, goal_embed_ang_sin, goal_embed_ang_cos,
